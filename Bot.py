...
# Состояния
VIN, MILEAGE, STYLE_CITY, STYLE_HIGHWAY, CITY, DELIVERY_TYPE, ADDRESS, PART_NODE, AXLE, PARTS, CONFIRM = range(11)
...
def get_order(order_number):
    ...
    if r:
        return {
            ...
            'total_price': r[21], 'our_cost': r[22], 'created_at': r[23]
        }
    return None
...
async def get_style_highway(upd, ctx):
    ...
    await upd.message.reply_text("🏙️ Введите ваш адрес (город, улица, дом):")
    return CITY
...
async def get_delivery_type(upd, ctx):
    ...
    if choice == "Курьером":
        ...
        await upd.message.reply_text(
            ...
            f"📍 Введите полный адрес доставки (город, улица, дом):"
        )
        return ADDRESS
...
async def get_parts(upd, ctx):
    ...
    summary += f"🚗 VIN: {data.get('vin', 'не указан')}\n"
    summary += f"📊 Пробег: {data.get('mileage', 'не указан')} км\n"
    summary += f"🏎️ Стиль город: {data.get('style_city', 'не указан')}\n"
    summary += f"🛣️ Стиль трасса: {data.get('style_highway', 'не указан')}\n"
    summary += f"🏙️ Адрес: {data.get('city', 'не указан')}\n"
    summary += f"🚚 Доставка: {data.get('delivery_type', 'не указана')}\n"
    summary += f"📍 Адрес доставки: {data.get('delivery_address', 'не указан')}\n"
    summary += f"🔧 Узел: {data.get('part_node', 'не указан')}\n"
    summary += f"🔧 Ось: {data.get('axle', 'не указана')}\n"
    summary += f"📝 Запчасти: {data.get('needed_parts', 'не указаны')}\n\n"
    summary += f"💰 Доставка: {data.get('delivery_price', 500)} руб.\n\n"
    summary += "✅ Всё верно? Нажмите «Готово» или «Редактировать»"
...
async def confirm_order(upd, ctx):
    if upd.message.text == "✅ Готово":
        ...
        await upd.context.bot.send_message(
            ...
            f"📍 Адрес доставки: {data.get('delivery_address','не указан')}\n"
...
async def view_order(upd, ctx):
    ...
    await q.answer()
    order_num = q.data[5:]
    ctx.user_data['current_order'] = order_num
    ...
    # Если заказ ещё не обработан или ожидает ответа, показываем кнопки управления
    if order['status'] in ['pending', 'waiting_answer', 'waiting_selection']:
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("✏️ Редактировать", callback_data="client_edit")],
            [InlineKeyboardButton("🗑️ Удалить", callback_data="client_delete")],
            [InlineKeyboardButton("🔍 Детали", callback_data="client_detail")],
            [InlineKeyboardButton("◀️ Назад", callback_data="back_orders")]
        ])
        await q.edit_message_text(text + "\n\n⚠️ Заказ ожидает обработки. Вы можете его изменить или удалить.", reply_markup=kb)
    else:
...
# ========== УПРАВЛЕНИЕ ЗАКАЗОМ КЛИЕНТОМ ==========
async def client_edit_order(upd, ctx):
    """Клиент хочет изменить заказ (начинает новый заказ)"""
    q = upd.callback_query
    await q.answer()
    
    order_num = ctx.user_data.get('current_order')
    
    if order_num:
        update_order(order_num, status='cancelled', status_text='❌ Отменён клиентом')
        order = get_order(order_num)
        if order:
            await upd.context.bot.send_message(
                MANAGER_ID,
                text=f"❌ КЛИЕНТ ОТМЕНИЛ ЗАКАЗ {order_num}\n👤 {order['user_name']}\nДля повторного оформления."
            )
    
    await q.edit_message_text("✏️ Давайте создадим новый заказ. Нажмите 🛒 Новый заказ", reply_markup=main_menu)
    await q.message.reply_text("🛒 Для оформления нового заказа нажмите кнопку ниже:", reply_markup=main_menu)

async def client_delete_order(upd, ctx):
    """Клиент удаляет свой заказ"""
    q = upd.callback_query
    await q.answer()
    ...
    if not order_num:
        await q.edit_message_text("❌ Ошибка: заказ не найден")
        return
    ...
    if order:
        await upd.context.bot.send_message(
            MANAGER_ID,
            text=f"🗑️ КЛИЕНТ УДАЛИЛ ЗАКАЗ {order_num}\n👤 {order['user_name']}"
        )
    
    await q.edit_message_text(f"✅ Заказ {order_num} успешно удалён!", reply_markup=main_menu)

async def client_detail_order(upd, ctx):
    """Детальный просмотр заказа для клиента"""
    q = upd.callback_query
    await q.answer()
    ...
    text += f"📦 Номер: {order['order_number']}\n"
    text += f"👤 Клиент: {order['user_name']}\n"
    text += f"📅 Создан: {order['created_at']}\n"
    text += f"🚗 VIN: {order['vin']}\n"
    text += f"📊 Пробег: {order['mileage']} км\n"
    text += f"🏙️ Адрес: {order['city']}\n"
    text += f"🏎️ Стиль город: {order['style_city']}\n"
    text += f"🛣️ Стиль трасса: {order['style_highway']}\n"
    text += f"🚚 Доставка: {order['delivery_type']} | {order['delivery_price']} руб.\n"
    text += f"📍 Адрес доставки: {order['delivery_address']}\n"
    text += f"🔧 Узел: {order['part_node']}\n"
    text += f"🔧 Ось: {order['axle']}\n"
    text += f"📝 Запчасти: {order['needed_parts']}\n\n"
    text += f"💰 Подбор менеджера:\n{order['selected_products'] or 'ещё не предложен'}\n\n"
    text += f"📦 Статус: {order['status_text']}"
    
    if order['tracking_number']:
        text += f"\n📦 Трек-номер: {order['tracking_number']}"
    
    back_kb = InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data=f"view_{order_num}")]])
...
# ========== ДЕТАЛЬНЫЙ ПРОСМОТР ДЛЯ АДМИНА ==========
async def admin_detail_order(upd, ctx):
    """Детальный просмотр заказа для админа"""
    q = upd.callback_query
    await q.answer()
    ...
    text += f"📦 Номер: {order['order_number']}\n"
    text += f"👤 Клиент: {order['user_name']}\n"
    text += f"🆔 ID клиента: {order['user_id']}\n"
    text += f"📅 Создан: {order['created_at']}\n"
    text += f"🚗 VIN: {order['vin']}\n"
    text += f"📊 Пробег: {order['mileage']} км\n"
    text += f"🏙️ Адрес: {order['city']}\n"
    text += f"🏎️ Стиль город: {order['style_city']}\n"
    text += f"🛣️ Стиль трасса: {order['style_highway']}\n"
    text += f"🚚 Доставка: {order['delivery_type']}\n"
    text += f"💰 Стоимость доставки: {order['delivery_price']} руб.\n"
    text += f"📍 Адрес доставки: {order['delivery_address']}\n"
    text += f"🔧 Узел: {order['part_node']}\n"
    text += f"🔧 Ось: {order['axle']}\n"
    text += f"📝 Запчасти:\n{order['needed_parts']}\n\n"
    text += f"💰 **Подбор менеджера:**\n{order['selected_products'] or 'ещё не предложен'}\n\n"
    text += f"💰 **Финансы:**\n"
    text += f"• Сумма запчастей: {int(order['total_price'])} руб.\n"
    text += f"• Доставка: {order['delivery_price']} руб.\n"
    text += f"• Итого: {int(total)} руб.\n"
    if our_cost:
        text += f"• Себестоимость: {int(our_cost)} руб.\n"
        text += f"• Маржа: {int(margin)} руб.\n"
    text += f"\n📦 Статус: {order['status_text']}"
...
async def admin_callback(upd, ctx):
    ...
    if data.startswith("admin_order_"):
        ...
        kb = [
            ...
            [InlineKeyboardButton("🔍 Детальный просмотр", callback_data=f"admin_detail_{order_num}")],
            [InlineKeyboardButton("🗑️ Удалить заказ", callback_data=f"delete_order_{order_num}")],
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(client_edit_order, pattern="^client_edit$"))
    app.add_handler(CallbackQueryHandler(client_delete_order, pattern="^client_delete$"))
    app.add_handler(CallbackQueryHandler(client_detail_order, pattern="^client_detail$"))
