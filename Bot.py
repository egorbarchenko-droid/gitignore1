...
# ========== АДМИН ПАНЕЛЬ ==========
async def show_admin_panel(q, message_text=None):
    """Показывает обновлённую админ-панель"""
...
async def admin_menu(upd, ctx):
    if upd.effective_user.id != MANAGER_ID:
        if hasattr(upd, 'callback_query') and upd.callback_query:
            ...
            await upd.callback_query.edit_message_text("⛔ Доступ запрещён")
        else:
    ...
    if hasattr(upd, 'callback_query') and upd.callback_query:
        await upd.callback_query.edit_message_text(text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
    else:
...
async def admin_callback(upd, ctx):
    ...
    if data == "admin_stats":
        ...
        await q.edit_message_text(f"📊 **СТАТИСТИКА**\n\n📦 Заказов: {total_orders}\n💰 Сумма: {int(total_sum)} руб.", parse_mode='Markdown')
        return
    ...
    if data == "admin_fix":
        ...
        await show_admin_panel(q, f"✅ Тестовый заказ {num} создан!")
        return
    ...
    if data.startswith("detail_"):
        ...
        text = (f"🔍 **ПОЛНАЯ ИНФОРМАЦИЯ**\n\n"
                ...
                f"💰 Сумма: {int(order.get('total_price', 0))} руб.\n"
    ...
    if data.startswith("confirm_delete_"):
        ...
        await show_admin_panel(q, f"✅ Заказ {order_num} УДАЛЁН!")
        return
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(admin_callback, pattern="^admin_|^pay_|^ship_|^del_|^edit_delivery_|^set_delivery_|^ask_|^detail_|^delete_|^confirm_delete_|^admin_back$"))
