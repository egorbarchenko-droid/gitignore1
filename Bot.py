...
def init_db():
    ...
    try:
        ...
        pass
    try:
        c.execute('ALTER TABLE orders ADD COLUMN our_cost REAL DEFAULT 0')
    except:
        pass
    try:
        ...
        pass
    try:
        c.execute('ALTER TABLE orders ADD COLUMN final_order TEXT')
    except:
        pass
...
def get_order(order_number):
    ...
    return {
        ...
        'vin': row[5] if len(row) > 5 else '',
        'mileage': row[6] if len(row) > 6 else '',
        'style_city': row[7] if len(row) > 7 else '',
        'style_highway': row[8] if len(row) > 8 else '',
        'city': row[9] if len(row) > 9 else '',
        'distance': row[10] if len(row) > 10 else 0,
        'delivery_type': row[11] if len(row) > 11 else '',
        'delivery_price': row[12] if len(row) > 12 else 500,
        'delivery_address': row[13] if len(row) > 13 else '',
        'part_node': row[14] if len(row) > 14 else '',
        'axle': row[15] if len(row) > 15 else '',
        'needed_parts': row[16] if len(row) > 16 else '',
        ...
        'total_price': row[22] if len(row) > 22 else 0,
        'our_cost': row[23] if len(row) > 23 else 0,
        'created_at': row[24] if len(row) > 24 else '',
    }
...
async def admin_callback(upd, ctx):
    ...
    await q.answer()
    data = q.data
    print(f"🔍 Админ callback: {data}")  # Для отладки
    ...
    if data == "admin_stats":
        ...
        for o in orders:
            order = get_order(o[0])
            if order:
                total_sum += order['total_price'] + order['delivery_price']
    ...
    if data.startswith("admin_order_"):
        order_num = data[12:]  # "admin_order_" = 12 символов
        order = get_order(order_num)
        if not order:
            await q.edit_message_text("❌ Заказ не найден")
            return
        
        total_sum = int(order['total_price'] + order['delivery_price'])
        text = (f"📋 ЗАКАЗ {order['order_number']}\n\n"
                f"👤 {order['user_name']}\n"
                f"📞 {order.get('phone', 'не указан')}\n"
                f"🏙️ {order['city']}\n"
                f"🚚 Доставка: {order['delivery_type']} | {order['delivery_price']} руб.\n"
    ...
    if data.startswith("pay_"):
        ...
        await q.edit_message_text(q.message.text + "\n\n✅ Статус: ОПЛАЧЕН", reply_markup=q.message.reply_markup)
        return
    ...
    if data.startswith("del_"):
        ...
        await q.edit_message_text(q.message.text + "\n\n✅ Статус: ДОСТАВЛЕН", reply_markup=q.message.reply_markup)
        return
