...
async def bonus_cmd(upd, ctx):
    ...
    await upd.message.reply_text(f"🎁 БОНУСЫ\n💰 Баланс: {int(bal)}\n📦 Накоплено: {int(total)} ₽\n⭐ Начисление: {percent}%", parse_mode='Markdown')
...
async def delivery_cmd(upd, ctx):
    await upd.message.reply_text(f"🚚 ДОСТАВКА\nБаза: {DELIVERY_BASE} ₽\n\n0 км (Москва): {DELIVERY_BASE} ₽\n1-50 км: {DELIVERY_BASE}+км×25\n51-100 км: {DELIVERY_BASE}+км×35\n101+ км: {DELIVERY_BASE}+км×50", parse_mode='Markdown')
...
async def edit_delivery(upd, ctx):
    ...
    if len(parts) > 3:
        ...
        await ctx.bot.send_message(order['user_id'], f"✏️ Доставка изменена на: {desc}")
        await q.edit_message_text(f"✅ Доставка изменена: {desc}\n💰 {price} ₽")
    else:
        await q.edit_message_text("✏️ Выберите способ доставки:", reply_markup=edit_delivery_kb(order_num))
...
async def admin_stats(upd, ctx):
    ...
    status_counts = {}
    for o in orders: status_counts[o[2]] = status_counts.get(o[2], 0) + 1
