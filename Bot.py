...
async def confirm_order(upd, ctx):
    if upd.message.text == "✅ Готово":
        ...
        ctx.user_data['last_order'] = order_num
        
        # Отправляем клиенту сообщение, что заказ принят (НЕ уведомляем админа!)
        await upd.message.reply_text(
        ...
        # ВАЖНО: НЕ ОТПРАВЛЯЕМ УВЕДОМЛЕНИЕ АДМИНУ ЗДЕСЬ!
...
async def manager_reply(upd, ctx):
    # Проверяем, что сообщение от админа и это ответ на сообщение
    if upd.effective_user.id != MANAGER_ID or not upd.message.reply_to_message:
        return
    
    # Ищем номер заказа в тексте сообщения, на которое отвечает админ
    ...
    if not match:
        await upd.message.reply_text("❌ Не удалось определить номер заказа. Убедитесь, что вы отвечаете на сообщение с заказом.")
        return
    ...
    await upd.message.reply_text(
        f"✅ Подбор запчастей для заказа {order_num} отправлен клиенту!\n\n"
        f"💰 Себестоимость: {int(total_cost)} руб.\n"
        f"💰 Продажная цена: {int(sum(p['price'] for p in products))} руб.\n"
        f"💵 Ожидаемая прибыль: {int(sum(p['price'] for p in products) - total_cost)} руб."
    )
...
async def finalize_cb(upd, ctx):
    ...
    result += "\n\n📞 Менеджер свяжется с вами для уточнения оплаты и доставки."
    
    await q.edit_message_text(result, parse_mode='Markdown')
    
    # Уведомляем админа, что клиент подтвердил заказ
    await ctx.bot.send_message(
        MANAGER_ID,
        f"✅ **ЗАКАЗ {order_num} ПОДТВЕРЖДЕН КЛИЕНТОМ!**\n\n"
        f"👤 Клиент: {order['user_name']}\n"
        f"💰 Сумма заказа: {int(final_total)} руб.\n"
        f"🎁 Бонусы: +{bonus}\n\n"
        f"📋 Выбранные позиции:\n" + 
        "\n".join([f"• {p['name']} — {int(p['price'])} руб." for p in selected])
    )
...
async def admin_menu(upd, ctx):
    ...
    keyboard.append([InlineKeyboardButton("➕ Создать тестовый заказ", callback_data="admin_fix")])
    
    if orders:
        keyboard.append([InlineKeyboardButton("📋 ПОСЛЕДНИЕ ЗАКАЗЫ", callback_data="noop")])
        for o in orders[:10]:
            order_num = o[0]
...
async def admin_callback(upd, ctx):
    ...
    if data == "admin_stats":
        ...
        await q.edit_message_text(
            ...
            f"💰 Общая сумма: {int(total_sum)} руб.\n\n"
            f"**По статусам:**\n{status_text}",
            parse_mode='Markdown'
        )
        return
    ...
    if data.startswith("edit_delivery_"):
        ...
        await q.edit_message_text("✏️ **Выберите новый способ доставки:**", reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')
        return
    ...
    if data.startswith("set_delivery_"):
        ...
        if order:
            ...
            await ctx.bot.send_message(
                order['user_id'],
                text=f"✏️ Менеджер изменил способ доставки для заказа {order_num}\n\nНовый способ: {new_delivery}\n{desc}"
            )
    ...
    if data.startswith("detail_"):
        ...
        text = (f"🔍 **ПОЛНАЯ ИНФОРМАЦИЯ**\n\n"
                ...
                f"🏙️ Город: {order.get('city', 'не указан')}\n"
                f"📍 Адрес: {order.get('delivery_address', 'не указан')}\n"
                f"🚚 Доставка: {order.get('delivery_type', 'не указана')}\n"
                f"💰 Стоимость доставки: {order.get('delivery_price', 0)} руб.\n"
    ...
    if data.startswith("confirm_delete_"):
        ...
        if order:
            try:
                await ctx.bot.send_message(order['user_id'], text=f"🗑️ Заказ {order_num} был удалён менеджером.")
            except:
                pass
    ...
    if data == "admin_back":
        ...
        await q.edit_message_text("👨‍💼 **АДМИН ПАНЕЛЬ**\n\nВыберите заказ:", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
        return
