...
async def new_order(upd, ctx):
    await upd.message.reply_text("🔧 Отправьте VIN номер (можно фото или текстом):")
    return VIN
...
async def get_vin(upd, ctx):
    ...
    if message.photo:
        await message.reply_text("📸 Обрабатываю фото VIN...")
        photo_file = await message.photo[-1].get_file()
        file_path = f"/tmp/vin_{upd.effective_user.id}.jpg"
        await photo_file.download_to_drive(file_path)
        try:
            result = reader.readtext(file_path, detail=0, paragraph=False)
            all_text = "".join(result).upper().replace(" ", "")
            vin_match = re.search(r'[A-HJ-NPR-Z0-9]{17}', all_text)
            if vin_match:
                vin_number = vin_match.group(0)
                ctx.user_data['vin'] = vin_number
                await message.reply_text(f"✅ Распознан VIN: `{vin_number}`", parse_mode='Markdown')
            else:
                await message.reply_text("❌ Не удалось распознать VIN. Введите вручную (17 символов):")
                return VIN
        except Exception as e:
            print(f"OCR error: {e}")
            await message.reply_text("⚠️ Ошибка распознавания. Введите VIN вручную:")
            return VIN
        finally:
...
def main():
    ...
    print(f"💾 База данных: {DB_PATH}")
    print("📸 VIN распознаётся по фото")
