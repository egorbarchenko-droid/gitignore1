...
def get_order(order_number):
    ...
    if r:
        return {'order_number': r[1], 'user_id': r[2], 'user_name': r[3], 'vin': r[4],
                ...
                'tracking_number': r[20], 'total_price': r[21], 'created_at': r[24]}
    return None
...
def admin_kb():
    ...
    kb = [[InlineKeyboardButton(f"{o[0]} | {o[1][:12]} | {o[2][:10]}", callback_data=f"adm_{o[0]}")] for o in orders[:10]]
...
async def new_order(upd, ctx): await upd.message.reply_text("🔧 VIN номер (17 символов):"); return VIN
...
async def get_style(upd, ctx): ctx.user_data['driving_style'] = upd.message.text; await upd.message.reply_text("🏙️ Город:"); return CITY
async def get_city(upd, ctx): ctx.user_data['city'] = upd.message.text; await upd.message.reply_text("🌆 Скорость в городе (км/ч):", reply_markup=speed_city_kb()); return SPEED_CITY
async def get_speed_city(upd, ctx):
    q = upd.callback_query; await q.answer()
    ctx.user_data['speed_city'] = int(q.data.split('_')[1])
    await q.edit_message_text(f"✅ Скорость в городе: {ctx.user_data['speed_city']} км/ч\n\n🛣️ Скорость на трассе (км/ч):", reply_markup=speed_highway_kb())
    return SPEED_HIGHWAY
async def get_speed_highway(upd, ctx):
    q = upd.callback_query; await q.answer()
    ctx.user_data['speed_highway'] = int(q.data.split('_')[1])
    await q.edit_message_text(f"✅ Скорость в городе: {ctx.user_data['speed_city']} км/ч\n✅ Скорость на трассе: {ctx.user_data['speed_highway']} км/ч\n\n📍 Расстояние от МКАД (км, 0 - Москва):")
    return DISTANCE
async def get_dist(upd, ctx):
    try:
        ctx.user_data['distance'] = int(upd.message.text)
        await upd.message.reply_text("🚚 Способ доставки:", reply_markup=delivery_kb)
...
async def my_orders(upd, ctx):
    ...
    else: await upd.message.reply_text("📭 Нет заказов")
...
async def finalize_cb(upd, ctx):
    ...
    update_order(order_num, 
        final_order=f"✅ ЗАКАЗ #{order_num}\n\n" + 
                    "\n".join([f"• {p['name']} — {int(p['price'])} ₽" for p in selected]) +
                    f"\n\n{delivery_text}" +
...
async def admin_callback(upd, ctx):
    ...
    if data == "admin_back":
        kb = admin_kb()
        if kb:
            await q.edit_message_text("👨‍💼 МЕНЮ АДМИНА", reply_markup=kb)
        else:
    ...
    if data.startswith("pay_"):
        ...
        order = get_order(order_num)
        if order:
            await ctx.bot.send_message(order['user_id'], f"✅ Заказ {order_num} оплачен!")
        ...
        order = get_order(order_num)
        if order:
            await ctx.bot.send_message(order['user_id'], f"🏠 Заказ {order_num} доставлен!")
        await q.edit_message_text(q.message.text + "\n\n✅ Статус: Доставлен")
        ...
        total_sum = sum(o[4] or 0 for o in orders)
        status_counts = {}
        for o in orders:
            status_counts[o[2]] = status_counts.get(o[2], 0) + 1
        text = f"📊 СТАТИСТИКА\n\n📦 Заказов: {total_orders}\n💰 Сумма: {int(total_sum)} ₽\n\nПо статусам:\n" + "\n".join([f"• {s}: {c}" for s,c in status_counts.items()])
        await q.edit_message_text(text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="admin_back")]]))
...
async def track_input(upd, ctx):
    ...
    if 'track_for' in ctx.user_data:
        ...
        order = get_order(order_num)
        if order:
            await ctx.bot.send_message(order['user_id'], f"📦 Заказ {order_num} отправлен!\nТрек: {upd.message.text}")
