...
# ========== ИЗМЕНЕНИЕ ДОСТАВКИ (только способы, которые не выбрал клиент) ==========
...
# ========== УДАЛЕНИЕ ЗАКАЗА ==========
async def delete_order(upd, ctx):
    q = upd.callback_query
    await q.answer()
    order_num = q.data.split('_')[2]
...
async def confirm_delete_order(upd, ctx):
    q = upd.callback_query
    await q.answer()
    order_num = q.data.split('_')[2]
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(confirm_delete_order, pattern="^confirm_delete_"))
