...
async def admin_callback(upd, ctx):
    ...
    if data.startswith("admin_order_"):
        ...
        text = (f"📋 **ЗАКАЗ {order['order_number']}**\n\n"
                ...
                f"📝 Нужные запчасти:\n{order.get('needed_parts', 'не указаны')}")
        ...
        kb = [
            ...
            [InlineKeyboardButton("🔍 Полная информация", callback_data=f"detail_{order_num}")],
            [InlineKeyboardButton("🗑️ Удалить заказ", callback_data=f"delete_{order_num}")],
    ...
    if data.startswith("set_delivery_"):
        ...
        if order:
            ...
            if new_delivery == "Курьером":
                ...
                price = 0
                desc = "Сторонняя фирма (стоимость рассчитает менеджер)"
    ...
    if data.startswith("pay_"):
        ...
        await q.edit_message_text(q.message.text + "\n\n✅ Статус изменён на: ОПЛАЧЕН")
        return
    
    # Отправлен - запрос трек-номера
    ...
    if data.startswith("detail_"):
        ...
        text = (f"🔍 **ПОЛНАЯ ИНФОРМАЦИЯ О ЗАКАЗЕ {order['order_number']}**\n\n"
                ...
                f"🔧 Узел: {order.get('part_node', 'не указан')}\n"
                f"🔧 Ось: {order.get('axle', 'не указана')}\n"
                f"💰 Сумма запчастей: {int(order.get('total_price', 0))} руб.\n"
    ...
    if data.startswith("delete_"):
        ...
        await q.edit_message_text(f"⚠️ **Вы уверены, что хотите удалить заказ {order_num}?**\n\nЭто действие нельзя отменить.", reply_markup=kb, parse_mode='Markdown')
        return
    ...
    if data == "admin_back":
        ...
        await q.edit_message_text("👨‍💼 **АДМИН ПАНЕЛЬ**\n\nВыберите заказ для управления:", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode='Markdown')
        return
