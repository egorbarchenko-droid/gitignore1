...
def init_db():
    ...
    # Создаём таблицу orders с правильной структурой
    ...
    # Проверяем структуру таблицы и выводим колонки для отладки
...
def update_order(order_number, **kwargs):
    ...
    for key, val in kwargs.items():
        if key in existing_columns:
            ...
            print(f"⚠️ Колонка {key} не существует в таблице orders")
...
def get_order(order_number):
    ...
    # Строим запрос с существующими колонками
    ...
    if r:
        ...
        for i, col in enumerate(columns):
            result[col] = r[i] if i < len(r) else None
        
        # Добавляем значения по умолчанию для отсутствующих колонок
        if 'our_cost' not in result:
            result['our_cost'] = 0
        if 'final_order' not in result:
