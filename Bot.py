...
def get_order(order_number):
    ...
    # Получаем все поля с правильной типизацией
    order = {
        'id': row[0],
        ...
        'mileage': row[6] or '',
        'style_city': row[7] or '',
        'style_highway': row[8] or '',
        'city': row[9] or '',
        'distance': int(row[10]) if row[10] else 0,
        'delivery_type': row[11] or '',
        'delivery_price': int(row[12]) if row[12] else 500,
        'delivery_address': row[13] or '',
        'part_node': row[14] or '',
        'axle': row[15] or '',
        'needed_parts': row[16] or '',
        ...
        'total_price': int(row[22]) if len(row) > 22 and row[22] else 0,
        'our_cost': int(row[23]) if len(row) > 23 and row[23] else 0,
