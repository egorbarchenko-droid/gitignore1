...
async def start(upd, ctx):
    if ctx.args and ctx.args[0].startswith('ref_'):
        ...
        if ref_id != upd.effective_user.id:
            ...
            await ctx.bot.send_message(ref_id, f"Новый друг! {upd.effective_user.full_name} перешёл по вашей ссылке!")
            await upd.message.reply_text("Получено 500 бонусов за регистрацию!")
    await upd.message.reply_text("Добро пожаловать в магазин автозапчастей!\n\nНажмите кнопку Новый заказ", reply_markup=main_menu)

async def new_order(upd, ctx): 
    await upd.message.reply_text("Введите VIN номер автомобиля (17 символов):")
    return VIN
...
async def get_mileage(upd, ctx): 
    ...
    await upd.message.reply_text("Выберите стиль вождения в городе:", reply_markup=city_style_kb)
    return STYLE_CITY

async def get_style_city(upd, ctx): 
    ctx.user_data['style_city'] = upd.message.text
    await upd.message.reply_text("Выберите стиль вождения на трассе:", reply_markup=highway_style_kb)
    return STYLE_HIGHWAY

async def get_style_highway(upd, ctx): 
    ctx.user_data['style_highway'] = upd.message.text
    await upd.message.reply_text("Введите ваш город:")
    return CITY

async def get_city(upd, ctx): 
    ctx.user_data['city'] = upd.message.text
    await upd.message.reply_text("Введите расстояние от МКАД в километрах:\n(0 - если Москва)")
    return DISTANCE
...
async def get_distance(upd, ctx):
    try:
        ...
        await upd.message.reply_text(f"Стоимость доставки: {delivery_price} руб.\n\nВведите список нужных запчастей (каждая с новой строки):")
        return PARTS
    except:
...
Накоплено заказов: {int(total)} руб.
Текущий процент начисления: {percent}%
...
Друг получает 500 приветственных бонусов за первый заказ!
...
от 15 000 руб. → 10%
от 20 000 руб. → 15%
и так до 100% (бесплатная доставка)

Пример: при заказе на 35 000 руб. скидка 30% на доставку"""
    await upd.message.reply_text(text)
...
async def manager_reply(upd, ctx):
    ...
    if not products: 
        await upd.message.reply_text("Не удалось распознать. Формат:\nНазвание = цена\nПример:\nМасло Ravenol 5W-30 = 3000")
        return
...
async def admin_callback(upd, ctx):
    ...
    if data.startswith("adm_"):
        ...
        kb = [
            ...
            [InlineKeyboardButton("Редактировать", callback_data=f"edit_order_{order_num}")],
    ...
    if data.startswith("edit_order_"):
        ...
        await q.edit_message_text("Редактирование заказа\n\nЧто хотите изменить?", reply_markup=InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("Назад", callback_data=f"adm_{order_num}")]
        ]))
        return
    
    if data.startswith("edit_delivery_"):
        order_num = data[14:]
        await q.edit_message_text("Выберите новый способ доставки:", reply_markup=InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("Назад", callback_data=f"edit_order_{order_num}")]
        ]))
        return
    ...
    if data.startswith("set_delivery_"):
        ...
        await ctx.bot.send_message(order['user_id'], text=f"Менеджер изменил способ доставки\n\nНовая стоимость: {price} руб.")
        await q.edit_message_text(f"Доставка изменена!\n\n{desc}\nСтоимость: {price} руб.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Назад", callback_data=f"adm_{order_num}")]]))
        return
    ...
    if data.startswith("pay_"):
        ...
        order = get_order(order_num)
        if order: await ctx.bot.send_message(order['user_id'], text=f"Заказ {order_num} доставлен! Спасибо за покупку!")
        await q.edit_message_text(q.message.text + "\n\nСтатус изменён: Доставлен")
        ...
        await q.edit_message_text(f"СТАТИСТИКА\n\nЗаказов: {total_orders}\nОбщая сумма: {int(total_sum)} руб.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Назад", callback_data="admin_back")]]))
...
async def edit_price_input(upd, ctx):
    ...
    if 'edit_price_order' in ctx.user_data:
        ...
        try:
            ...
            order = get_order(order_num)
            await ctx.bot.send_message(order['user_id'], text=f"Менеджер изменил стоимость доставки\n\nНовая стоимость: {new_price} руб.")
...
async def fix_orders(upd, ctx):
    ...
    if c.fetchone()[0] == 0:
        ...
        await upd.message.reply_text(f"В базе {c.fetchone()[0]} заказов\n/menu")
    conn.close()
...
def main():
    ...
    print("Команды для админа: /menu, /fix")
    app.run_polling()
