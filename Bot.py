...
# Состояния
VIN, MILEAGE, STYLE_CITY, STYLE_HIGHWAY, CITY, DISTANCE, PARTS = range(7)
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS orders (
        ...
        style_city TEXT, style_highway TEXT,
...
def save_order(data):
    ...
    c.execute('''INSERT INTO orders (order_number, user_id, user_name, vin, mileage,
        style_city, style_highway, city, distance, delivery_price, needed_parts,
        status, status_text, created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
        (num, data['user_id'], data['user_name'], data['vin'], data['mileage'],
         data['style_city'], data['style_highway'], data['city'], data.get('distance',0),
         data.get('delivery_price',500), data['needed_parts'],
...
def get_order(order_number):
    ...
    if r:
        return {'order_number': r[1], 'user_id': r[2], 'user_name': r[3], 'vin': r[4],
                ...
                'distance': r[9], 'delivery_price': r[10], 'needed_parts': r[11],
                'selected_products': r[12], 'final_order': r[13], 'status': r[14],
                'status_text': r[15], 'tracking_number': r[16], 'total_price': r[17],
                'created_at': r[20]}
    return None
...
# Клавиатуры для стилей вождения (по 6 режимов)
city_style_kb = ReplyKeyboardMarkup([
    ["🚗 Очень спокойный (до 40 км/ч)"],
    ["🚗 Спокойный (40-60 км/ч)"],
    ["⚡ Умеренный (60-80 км/ч)"],
    ["🏎️ Активный (80-100 км/ч)"],
    ["🔥 Спортивный (100-120 км/ч)"],
    ["💨 Экстремальный (120+ км/ч)"]
], resize_keyboard=True)

highway_style_kb = ReplyKeyboardMarkup([
    ["🚗 Очень спокойный (до 70 км/ч)"],
    ["🚗 Спокойный (70-90 км/ч)"],
    ["⚡ Умеренный (90-110 км/ч)"],
    ["🏎️ Активный (110-130 км/ч)"],
    ["🔥 Спортивный (130-150 км/ч)"],
    ["💨 Экстремальный (150+ км/ч)"]
], resize_keyboard=True)
...
def admin_kb():
    ...
    kb = [[InlineKeyboardButton(f"📦 {o[0]} | {o[1][:12]}", callback_data=f"adm_{o[0]}")] for o in orders[:10]]
...
async def get_distance(upd, ctx):
    try:
        ...
        ctx.user_data['delivery_price'] = delivery_price
        await upd.message.reply_text(f"🚚 **Стоимость доставки:** {delivery_price} ₽\n\n🔧 **Какие запчасти нужны?** (напишите список)")
        return PARTS
    except:
...
async def get_parts(upd, ctx):
    ...
    order_num = save_order({
        ...
        'delivery_price': data.get('delivery_price',500), 'needed_parts': upd.message.text
    })
    await upd.message.reply_text(f"✅ **Заказ #{order_num} создан!**\n\n🚚 Доставка: {data.get('delivery_price',500)} ₽\n\nОжидайте подбора (15-30 мин)", reply_markup=main_menu, parse_mode='Markdown')
    await upd.context.bot.send_message(MANAGER_ID, f"🆕 **НОВЫЙ ЗАКАЗ #{order_num}**\n👤 {upd.effective_user.full_name}\n🚗 VIN: {data['vin']}\n📊 Пробег: {data['mileage']} км\n🏙️ Город: {data['city']} | {data.get('distance',0)} км от МКАД\n🏎️ Стиль город: {data['style_city']}\n🛣️ Стиль трасса: {data['style_highway']}\n🚚 Доставка: {data.get('delivery_price',500)} ₽\n📝 {upd.message.text}", parse_mode='Markdown')
    return ConversationHandler.END

async def my_orders(upd, ctx):
    kbd = get_orders_keyboard(upd.effective_user.id)
    if kbd: await upd.message.reply_text("📦 **ВАШИ ЗАКАЗЫ:**", reply_markup=kbd, parse_mode='Markdown')
...
async def view_order(upd, ctx):
    ...
    await q.edit_message_text(text, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="back_orders")]]))

async def back_orders(upd, ctx):
    q = upd.callback_query; await q.answer()
    kbd = get_orders_keyboard(q.from_user.id)
    if kbd: await q.edit_message_text("📦 **ВАШИ ЗАКАЗЫ:**", reply_markup=kbd, parse_mode='Markdown')
...
async def manager_reply(upd, ctx):
    ...
    if not products: await upd.message.reply_text("❌ Формат: Название = цена\nПример:\nМасло Ravenol 5W-30 = 3000"); return
    update_order(order_num, selected_products=upd.message.text, status='waiting_selection', status_text='🟡 Ожидает выбора')
    order = get_order(order_num)
    if order:
        kb = [[InlineKeyboardButton(f"⬜ {p['name']} — {int(p['price'])} ₽", callback_data=f"sel_{order_num}_{i}")] for i,p in enumerate(products)]
        kb.append([InlineKeyboardButton("✅ ПОДТВЕРДИТЬ", callback_data=f"fin_{order_num}")])
        await ctx.bot.send_message(order['user_id'], text=f"🛒 **Выберите запчасти для заказа {order_num}**\n\n(нажмите на нужные позиции, затем «ПОДТВЕРДИТЬ»)", reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')
...
async def finalize_cb(upd, ctx):
    ...
    if delivery_disc >= 100:
        ...
        delivery_text = f"🚚 Доставка: {delivery_price} ₽ → скидка {delivery_disc}% = {delivery_final} ₽"
    else:
...
async def admin_menu(upd, ctx):
    ...
    if not kb:
        ...
        return
    await upd.message.reply_text("👨‍💼 **МЕНЮ АДМИНИСТРАТОРА**\n\nВыберите заказ:", reply_markup=kb, parse_mode='Markdown')
...
async def admin_callback(upd, ctx):
    ...
    if data == "adm_fix":
        ...
        c.execute('''INSERT INTO orders (order_number, user_id, user_name, vin, mileage,
            ...
            (num, MANAGER_ID, 'Тестовый Клиент', 'TEST123', '50000',
             '🚗 Спокойный', '🚗 Спокойный', 'Москва', 0, 500,
    ...
    if data.startswith("set_delivery_"):
        ...
        update_order(order_num, delivery_price=price)
        await ctx.bot.send_message(order['user_id'], text=f"✏️ **Менеджер изменил способ доставки**\n\nНовая стоимость: {price} ₽")
        await q.edit_message_text(f"✅ Доставка изменена!\n\n🚚 {desc}\n💰 Стоимость: {price} ₽", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data=f"adm_{order_num}")]]))
        return
    ...
    if data == "admin_back":
        kb = admin_kb()
        if kb: await q.edit_message_text("👨‍💼 **МЕНЮ АДМИНИСТРАТОРА**\n\nВыберите заказ:", reply_markup=kb, parse_mode='Markdown')
        else: await q.edit_message_text("📭 Нет заказов")
        return
    ...
    if data.startswith("pay_"):
        ...
        order = get_order(order_num)
        if order: await ctx.bot.send_message(order['user_id'], text=f"✅ **Заказ {order_num} оплачен!**", parse_mode='Markdown')
        ...
        order = get_order(order_num)
        if order: await ctx.bot.send_message(order['user_id'], text=f"🏠 **Заказ {order_num} доставлен!**\n\nСпасибо за покупку!", parse_mode='Markdown')
        ...
        await q.edit_message_text(f"📊 **СТАТИСТИКА**\n\n📦 Заказов: {total_orders}\n💰 Общая сумма: {int(total_sum)} ₽", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="admin_back")]]), parse_mode='Markdown')
...
async def edit_price_input(upd, ctx):
    ...
    if 'edit_price_order' in ctx.user_data:
        ...
        try:
            ...
            await upd.message.reply_text(f"✅ Стоимость доставки для заказа {order_num} изменена на {new_price} ₽")
        except:
...
async def fix_orders(upd, ctx):
    ...
    if c.fetchone()[0] == 0:
        ...
        await upd.message.reply_text(f"📊 Заказы уже есть\n/menu")
    conn.close()
