...
MANAGER_ID = 804070528  # ВАШ ID АДМИНА
DELIVERY_BASE = 500

# Путь к постоянному хранилищу на Bothost
DB_PATH = "/data/shop_bot.db"
# =================================
...
• 20 000 ₽ → скидка 15%
• 25 000 ₽ → скидка 20%
... до 100% при заказе от 100 000 ₽

📌 **Пример:** Заказ на 35 000 ₽ + доставка 500 ₽
→ скидка 30% → доставка 350 ₽
...
async def admin_cmd(upd, ctx):
    ...
    if upd.effective_user.id != MANAGER_ID:
        await upd.message.reply_text("⛔ Доступ запрещён. Эта команда только для администратора.")
        return
    kb = admin_kb()
    if not kb:
        await upd.message.reply_text("📭 Нет заказов\nСоздайте тестовый заказ через /fix")
        return
    await upd.message.reply_text("👨‍💼 **АДМИН-ПАНЕЛЬ**\n\nВыберите заказ для управления:", parse_mode='Markdown', reply_markup=kb)

async def admin_stats(upd, ctx):
    """Статистика по заказам"""
    if upd.effective_user.id != MANAGER_ID:
        await upd.answer("⛔ Доступ запрещён")
        return
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
...
async def fix_cmd(upd, ctx):
    ...
    c.execute('''INSERT INTO orders (order_number, user_id, user_name, vin, mileage,
        ...
        needed_parts, status, status_text, created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
        (test_order_num, MANAGER_ID, "Тестовый пользователь", "TEST123", "50000",
         "Спокойный", "120", "Москва", 0, "Курьером", 500, "🚚 Курьер: 0 км от МКАД = 500 ₽",
         "Тестовый заказ для отладки", 'pending', '🟡 Ожидает подбора', datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
    conn.commit()
    conn.close()
...
def main():
    ...
    # Обработчики сообщений от админа
