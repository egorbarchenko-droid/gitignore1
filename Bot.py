...
def extract_city_from_address(address):
    ...
    cities = {
        ...
        'реутов': 'Реутов', 'балашиха': 'Балашиха', 'королёв': 'Королёв',
        'подольск': 'Подольск', 'троицк': 'Троицк', 'щелково': 'Щелково'
    }
...
def extract_distance_from_address(address):
    ...
    distances = {
        ...
        'красногорск': 7, 'одинцово': 10,
        'реутов': 8, 'балашиха': 15, 'королёв': 18,
        'подольск': 25, 'троицк': 35, 'щелково': 20
    }
...
def get_status_bar(status_text):
    statuses = [("Ожидает подбора", 1), ("Ожидает выбора", 2), ("Оплачен", 3), ("Отправлен", 4), ("Доставлен", 5)]
    current = 0
...
async def start(upd, ctx):
    ...
    text = ("🏎️ Добро пожаловать в магазин автозапчастей!\n\n"
            "🛒 Новый заказ - подбор запчастей\n"
            "🚗 Мой гараж - храните авто\n"
            "📦 Мои заказы - история\n"
            "🎁 Бонусы - накапливайте\n"
            "🔗 Рефералы - приглашайте\n"
            "🚚 Доставка - расчёт\n\n"
            "Нажмите 🛒 Новый заказ!")
...
async def new_order(upd, ctx):
    ...
    if cars:
        ...
        await upd.message.reply_text("🔧 ВЫБЕРИТЕ АВТОМОБИЛЬ:", reply_markup=InlineKeyboardMarkup(keyboard))
        return VIN
    else:
...
async def order_auto_callback(upd, ctx):
    ...
    if q.data == "order_manual":
        ...
        await q.edit_message_text(f"🚗 Выбран: {vin}\n\n📊 Введите пробег (км):")
        return MILEAGE

async def get_vin(upd, ctx):
    if upd.message.photo:
        await upd.message.reply_text("📸 Введите VIN вручную (17 символов):")
        return VIN
    vin = upd.message.text.upper().strip()
    if len(vin) != 17:
        await upd.message.reply_text("❌ VIN должен быть 17 символов:")
        return VIN
    ctx.user_data['vin'] = vin
    await upd.message.reply_text(f"{get_order_progress(0)}\n\n📊 Введите пробег (км):")
    return MILEAGE
...
async def get_address(upd, ctx):
    ...
    ctx.user_data['city'] = extract_city_from_address(full_address)
    ctx.user_data['distance'] = extract_distance_from_address(full_address)
    await upd.message.reply_text(f"{get_order_progress(4)}\n\n📍 Адрес: {full_address}\n🏙️ Город: {ctx.user_data['city']}\n📏 Расстояние: {ctx.user_data['distance']} км\n💡 Стоимость рассчитает менеджер\n\n📞 Введите телефон:")
    return PHONE

async def get_phone(upd, ctx):
    phone = upd.message.text.strip()
    if len(phone) < 5:
        await upd.message.reply_text("❌ Введите корректный телефон:")
        return PHONE
    ctx.user_data['phone'] = phone
    await upd.message.reply_text(f"{get_order_progress(5)}\n\n🔧 Выберите узел:", reply_markup=part_node_kb)
    return PART_NODE

async def get_part_node(upd, ctx):
    ctx.user_data['part_node'] = upd.message.text
    if upd.message.text in AXLE_REQUIRED_NODES:
        await upd.message.reply_text(f"{get_order_progress(6)}\n\n🔧 Выберите ось:", reply_markup=axle_kb)
        return AXLE
    else:
...
async def get_parts(upd, ctx):
    if not upd.message.text.strip():
        await upd.message.reply_text("❌ Введите запчасти:")
        return PARTS
    ctx.user_data['needed_parts'] = upd.message.text
    
    data = ctx.user_data
    summary = (f"{get_order_progress(7)}\n\n📋 ПРОВЕРЬТЕ ЗАКАЗ\n\n"
               f"🚗 VIN: {data.get('vin', '-')}\n"
               ...
               f"🔧 Узел: {data.get('part_node', '-')}\n"
               f"🔧 Ось: {data.get('axle', '-')}\n"
               f"📝 Запчасти: {data.get('needed_parts', '-')}\n\n"
               f"💰 Доставка: будет рассчитана\n\n"
               "✅ Всё верно?")
...
async def confirm_order(upd, ctx):
    if upd.message.text == "✅ Готово":
        ...
        await upd.context.bot.send_message(
            ...
            f"➡️ Ответьте на это сообщение для подбора"
        )
        ...
        await upd.message.reply_text("✏️ Начнём заказ заново. Нажмите 🛒 Новый заказ", reply_markup=main_menu)
        return ConversationHandler.END
...
async def my_orders(upd, ctx):
    ...
    for o in orders:
        ...
        total = int(o[3]) + int(o[4]) if o[3] and o[4] else 0
...
async def view_order(upd, ctx):
    ...
    text = (f"📋 ЗАКАЗ {order['order_number']}\n\n{status_bar}\n\n"
            f"👤 {order['user_name']}\n📅 {order['created_at']}\n"
            f"🚗 VIN: {order.get('vin', '-')}\n📊 {order.get('mileage', '-')} км\n"
            f"🏙️ {order.get('city', '-')}\n🚚 {order.get('delivery_type', '-')} | {order.get('delivery_price', 0)} руб.\n"
            f"💰 Сумма: {total} руб.\n📦 Статус: {order.get('status_text', '-')}")
    
    if order.get('tracking_number'):
        text += f"\n📮 Трек: {order.get('tracking_number')}"
    
    if order.get('final_order') and order['final_order'] not in [None, '[]']:
        text += "\n\n📦 ЗАКАЗАННЫЕ ЗАПЧАСТИ:\n"
        try:
            parts = ast.literal_eval(order['final_order'])
            for p in parts[:10]:
                if isinstance(p, dict):
                    text += f"• {p.get('name', '-')} — {int(p.get('price', 0))} руб.\n"
        except:
            text += f"{order['final_order'][:300]}"
    
    kb = [[InlineKeyboardButton("◀️ Назад", callback_data="back_orders")]]
...
async def bonus_cmd(upd, ctx):
    ...
    text = (f"🎁 БОНУСЫ\n\n💰 Баланс: {int(b['balance'])}\n"
            f"📈 Начислено: {int(b['total_earned'])}\n"
            f"📉 Потрачено: {int(b['total_spent'])}\n"
    ...
    if history:
        ht = ""
        for h in history[:10]:
            ht += f"\n• {int(h[1])} руб. | {'Начисление' if h[2]=='earned' else 'Списание'}\n  {h[4][:16]}\n"
        await upd.message.reply_text(ht[:4000])
    else:
        await upd.message.reply_text("📭 История пуста")

async def referral_cmd(upd, ctx):
    bot_name = (await ctx.bot.get_me()).username
    link = f"https://t.me/{bot_name}?start=ref_{upd.effective_user.id}"
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    ...
    conn.close()
    text = (f"🔗 РЕФЕРАЛЫ\n\n{link}\n\n👥 Приглашено: {count}\n"
            "📊 Вы получаете 0.5% от покупок друзей!\n🎁 Друг получает 500 бонусов!")
    await upd.message.reply_text(text)

async def delivery_cmd(upd, ctx):
    text = (f"🚚 ДОСТАВКА ОТ МКАД\n\nБаза: {DELIVERY_BASE} руб.\n\n"
            "0 км: 500 руб.\n1-50 км: +25 руб/км\n51-100 км: +35 руб/км\n101+ км: +50 руб/км\n\n"
            "Самовывоз: бесплатно (Давыдково, Строгино, Южная)\n\n"
            "Скидка на доставку от суммы заказа (до 100%)")
    await upd.message.reply_text(text)
...
async def manager_reply(upd, ctx):
    ...
    if order:
        ...
        await ctx.bot.send_message(order['user_id'], text=f"🛒 ПОДБОР ДЛЯ ЗАКАЗА #{order_num}\n\nВыберите:", reply_markup=InlineKeyboardMarkup(kb))
        await upd.message.reply_text(f"✅ Подбор для {order_num} отправлен!")
...
async def finalize_cb(upd, ctx):
    ...
    if delivery_disc >= 100:
        ...
        discount = int(delivery_price * delivery_disc / 100)
        delivery_final = delivery_price - discount
...
async def send_question_to_client(upd, ctx):
    ...
    await ctx.bot.send_message(order['user_id'], text=f"❓ Вопрос по заказу {order_num}\n\n{question}")
    await upd.message.reply_text(f"✅ Вопрос отправлен")
...
async def client_answer(upd, ctx):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute('SELECT order_number FROM orders WHERE user_id = ? AND status = "waiting_answer" LIMIT 1', (uid,))
    row = c.fetchone()
    conn.close()
    if row:
        order_num = row[0]
...
async def admin_menu(upd, ctx, callback_query=None):
    ...
    total = len(orders)
    new_orders = sum(1 for o in orders if 'Ожидает подбора' in o[2])
    in_work = sum(1 for o in orders if 'Ожидает ответа' in o[2] or 'Ожидает выбора' in o[2])
    paid = sum(1 for o in orders if 'Оплачен' in o[2])
    shipped = sum(1 for o in orders if 'Отправлен' in o[2])
    delivered = sum(1 for o in orders if 'Доставлен' in o[2])
    unread = sum(1 for o in orders if len(o) > 4 and o[4] == 0)
    
    keyboard = []
    keyboard.append([
        InlineKeyboardButton(f"🆕 Новые: {new_orders}", callback_data="filter_🆕"),
        InlineKeyboardButton(f"🟡 В работе: {in_work}", callback_data="filter_🟡"),
        InlineKeyboardButton(f"💰 Оплачен: {paid}", callback_data="filter_💰"),
        InlineKeyboardButton(f"🚚 Отправлен: {shipped}", callback_data="filter_🚚"),
        InlineKeyboardButton(f"✅ Доставлен: {delivered}", callback_data="filter_✅")
    ])
    keyboard.append([
    ...
    if unread > 0:
        keyboard.append([InlineKeyboardButton(f"📬 Непрочитанные ({unread})", callback_data="filter_unread")])
...
async def admin_dashboard(upd, ctx):
    ...
    stats = {'total': len(orders), 'week': 0, 'month': 0, 'sum': 0, 'week_sum': 0, 'month_sum': 0}
    for o in orders:
        order = get_order(o[0])
        if order:
    ...
    text = (f"📊 ДАШБОРД\n\n📦 Всего: {stats['total']}\n💰 Сумма: {int(stats['sum'])} руб.\n\n"
            f"📅 За неделю: {stats['week']} ({int(stats['week_sum'])} руб.)\n"
            f"📅 За месяц: {stats['month']} ({int(stats['month_sum'])} руб.)")
...
async def admin_search_input(upd, ctx):
    ...
    for order in results[:10]:
        icon = "💰" if 'Оплачен' in order['status_text'] else "📦"
...
async def admin_callback(upd, ctx):
    ...
    if data == "admin_fix":
        ...
        c.execute('''INSERT INTO orders (
            ...
            (num, MANAGER_ID, 'Тестовый', 'TEST123', '50000',
    ...
    if data.startswith("del_"):
        ...
        order = get_order(order_num)
        if order:
            await ctx.bot.send_message(order['user_id'], text=f"🏠 Заказ {order_num} доставлен!")
        await q.edit_message_text(q.message.text + "\n\n✅ ДОСТАВЛЕН")
        return
    ...
    if data.startswith("confirm_delete_"):
        ...
        conn.commit()
        conn.close()
        print(f"✅ Заказ {order_num} удалён")
...
async def set_delivery_price_input(upd, ctx):
    ...
    try:
        ...
        order = get_order(order_num)
        await ctx.bot.send_message(order['user_id'], text=f"💰 Стоимость доставки для {order_num}: {new_price} руб.")
        await upd.message.reply_text(f"✅ Цена установлена: {new_price} руб.")
    except:
...
async def garage_menu(upd, ctx):
    cars = get_cars(upd.effective_user.id)
    if not cars:
        await upd.message.reply_text("🚗 МОЙ ГАРАЖ\n\nУ вас пока нет автомобилей.\n\n➕ Добавить: отправьте VIN",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("➕ Добавить авто", callback_data="garage_add")],
                [InlineKeyboardButton("◀️ Назад", callback_data="garage_back")]
            ]))
        return
    text = "🚗 МОЙ ГАРАЖ\n\n"
    for car in cars:
        text += f"🔹 {car[0]}\n   {car[1][:30] if car[1] else 'без описания'}\n   📅 {car[2][:10]}\n\n"
    keyboard = [[InlineKeyboardButton(f"🗑️ Удалить {car[0]}", callback_data=f"garage_del_{car[0]}")] for car in cars]
    keyboard.append([InlineKeyboardButton("➕ Добавить авто", callback_data="garage_add")])
    keyboard.append([InlineKeyboardButton("◀️ Назад", callback_data="garage_back")])
    await upd.message.reply_text(text, reply_markup=InlineKeyboardMarkup(keyboard))

async def garage_add_start(upd, ctx):
    q = upd.callback_query
    await q.answer()
    await q.edit_message_text("🚗 ДОБАВЛЕНИЕ\n\nОтправьте VIN номер (17 символов):")
    return GARAGE_VIN
...
async def garage_get_description(upd, ctx):
    desc = upd.message.text.strip()
    if desc == "-":
        desc = ""
    vin = ctx.user_data['garage_vin']
    save_car(upd.effective_user.id, vin, desc)
