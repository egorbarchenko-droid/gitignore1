...
# ========== НАСТРОЙКИ ==========
BOT_TOKEN = "8731285343:AAFr9zDJF3RPVM_wsOSHvIRxe6sb8eMQrqM"
...
# ========== КЛАВИАТУРЫ ==========
main_menu = ReplyKeyboardMarkup([["?? Новый заказ"], ["?? Мои заказы", "?? Бонусы"], ["?? Рефералы", "?? Доставка"], ["?? Помощь"]], resize_keyboard=True)
style_kb = ReplyKeyboardMarkup([["?? Спокойный", "? Умеренный"], ["??? Активный", "?? Спортивный"]], resize_keyboard=True)
...
async def start(upd, ctx):
    ...
    await upd.message.reply_text("??? Добро пожаловать!\nНажмите ?? Новый заказ", reply_markup=main_menu)
...
async def get_mileage(upd, ctx): ctx.user_data['mileage'] = upd.message.text; await upd.message.reply_text("??? Стиль езды:", reply_markup=style_kb); return DRIVING_STYLE
async def get_style(upd, ctx): ctx.user_data['driving_style'] = upd.message.text; await upd.message.reply_text("??? **Введите город**"); return CITY
...
async def admin_order(upd, ctx):
    ...
    kb = [
        ...
        [InlineKeyboardButton("?? Изменить доставку", callback_data=f"edit_delivery_{order['order_number']}")],
...
async def edit_delivery(upd, ctx):
    ...
    if len(parts) > 3:
        ...
        await ctx.bot.send_message(order['user_id'], f"?? Менеджер изменил способ доставки на: {desc}")
        await q.edit_message_text(f"? Доставка изменена на: {desc}\n\n?? Стоимость: {price} ?")
    else:
        await q.edit_message_text("?? **Выберите новый способ доставки:**", reply_markup=edit_delivery_kb(order_num))
