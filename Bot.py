...
def init_db():
    ...
    # Добавляем колонку total_spent если её нет
    try:
...
# ========== ФУНКЦИИ ДЛЯ БОНУСОВ ==========
def spend_bonus(user_id, amount, order_num, desc):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
...
async def garage_get_description(upd, ctx):
    ...
    await upd.message.reply_text(
        f"✅ Автомобиль `{vin}` успешно добавлен в ваш гараж!",
        parse_mode='Markdown'
    )
...
async def garage_delete(upd, ctx):
    ...
    if delete_car(q.from_user.id, vin):
        await q.edit_message_text(f"✅ Автомобиль `{vin}` удалён из гаража!", parse_mode='Markdown')
    else:
        await q.edit_message_text(f"❌ Автомобиль `{vin}` не найден в гараже.", parse_mode='Markdown')
...
async def view_order(upd, ctx):
    ...
    # Показываем заказанные запчасти
    ...
    if bonus_row:
        text += f"\n\n🎁 **НАЧИСЛЕНО БОНУСОВ:** +{int(bonus_row[0])} руб."
...
async def finalize_cb(upd, ctx):
    ...
    # ✅ УВЕДОМЛЕНИЕ АДМИНУ О ПОДТВЕРЖДЕНИИ КЛИЕНТА
    await ctx.bot.send_message(
        MANAGER_ID,
