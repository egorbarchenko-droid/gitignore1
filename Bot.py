...
async def admin_menu(upd, ctx, callback_query=None):
    ...
    if orders:
        ...
        keyboard.append([InlineKeyboardButton("📭 Нет заказов", callback_data="noop")])
    
    text = f"👨‍💼 **АДМИН ПАНЕЛЬ**\n\n📊 Всего заказов: {total_orders}\n"
    text += f"🆕 Новых: {new_orders}\n"
    text += f"🟡 В работе: {in_work}\n"
    text += f"💰 Оплаченных: {paid}\n"
    text += f"🚚 Отправленных: {shipped}\n"
    text += f"✅ Доставленных: {delivered}\n"
    if unread > 0:
...
async def admin_callback(upd, ctx):
    ...
    if data.startswith("edit_delivery_"):
        ...
        await q.edit_message_text("✏️ **ВЫБЕРИТЕ ДОСТАВКУ:**", reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')
        return
