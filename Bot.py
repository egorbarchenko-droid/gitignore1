...
# Состояния - УБРАЛИ ОТДЕЛЬНЫЙ ШАГ С ГОРОДОМ
...
# НОВАЯ ФУНКЦИЯ - пытается определить город из адреса
...
# НОВАЯ ФУНКЦИЯ - пытается определить расстояние из адреса
...
async def get_delivery_type(upd, ctx):
    ...
    if choice == "Курьером":
        ...
        await upd.message.reply_text(
            ...
            "Адрес будет автоматически обработан (город, расстояние от МКАД)"
        )
        return ADDRESS
...
async def get_address(upd, ctx):
    ...
    # Автоматически определяем город и расстояние
    ...
    # Рассчитываем стоимость доставки если выбран курьер
...
async def admin_callback(upd, ctx):
    ...
    if data.startswith("delete_"):
        ...
        await q.edit_message_text(f"⚠️ **Удалить заказ {order_num}?**", reply_markup=kb, parse_mode='Markdown')
        return
