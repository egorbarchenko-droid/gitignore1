...
QUICK_REPLIES = {
    "specify_car": "Уточните, пожалуйста, марку и год выпуска автомобиля",
    "original_or_analog": "Вам нужен оригинал или аналог?",
    "in_stock": "Данная запчасть есть в наличии. Стоимость:",
    "need_order": "Запчасть под заказ, срок поставки 3-5 дней",
}

# ========== ФУНКЦИИ КЭШИРОВАНИЯ ==========
def cache_get(key, ttl_seconds=3600):
    cache_file = os.path.join(CACHE_DIR, f"{hashlib.md5(key.encode()).hexdigest()}.json")
    if os.path.exists(cache_file):
        with open(cache_file, 'r') as f:
            data = json.load(f)
        if datetime.fromisoformat(data['timestamp']) > datetime.now() - timedelta(seconds=ttl_seconds):
            return data['value']
    return None
...
def init_db():
    ...
    # Добавляем колонку viewed если её нет
    try:
...
async def garage_delete_confirm(upd, ctx):
    q = upd.callback_query
    await q.answer()
    vin = q.data[12:]
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Да, удалить", callback_data=f"garage_del_yes_{vin}")],
        [InlineKeyboardButton("❌ Нет, отмена", callback_data="garage_back")]
    ])
    await q.edit_message_text(f"⚠️ Удалить автомобиль {vin} из гаража?", reply_markup=kb)

async def garage_delete(upd, ctx):
    q = upd.callback_query
    await q.answer()
    vin = q.data[16:]  # убираем "garage_del_yes_"
...
async def my_orders(upd, ctx):
    ...
    for o in orders:
        ...
        kb.append([InlineKeyboardButton(f"🔄 Повторить заказ {order_num}", callback_data=f"repeat_{order_num}")])
...
async def view_order(upd, ctx):
    ...
    kb = [
        [InlineKeyboardButton("🔄 Повторить заказ", callback_data=f"repeat_{order_num}")],
        [InlineKeyboardButton("◀️ Назад к списку", callback_data="back_orders")]
    ]
    await q.edit_message_text(text, reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')

async def repeat_order(upd, ctx):
    q = upd.callback_query
    await q.answer()
    order_num = q.data[7:]
    old_order = get_order(order_num)
    
    if not old_order:
        await q.edit_message_text("❌ Заказ не найден")
        return
    
    # Копируем данные из старого заказа
    ctx.user_data['vin'] = old_order['vin']
    ctx.user_data['mileage'] = old_order['mileage']
    ctx.user_data['style_city'] = old_order['style_city']
    ctx.user_data['style_highway'] = old_order['style_highway']
    
    await q.edit_message_text(
        f"🔄 Данные скопированы из заказа {order_num}!\n\n"
        f"🚗 VIN: {old_order['vin']}\n"
        f"📊 Пробег: {old_order['mileage']} км\n"
        f"🏎️ Стиль город: {old_order['style_city']}\n"
        f"🛣️ Стиль трасса: {old_order['style_highway']}\n\n"
        f"Вы можете изменить пробег или отправить '=' чтобы оставить как есть:"
    )
    return MILEAGE
...
async def help_cmd(upd, ctx):
    text = ("📖 **ПОМОЩЬ**\n\n"
            ...
            "**🔄 Повтор заказа:**\n"
            "В истории заказов есть кнопка 'Повторить заказ'\n\n"
            "**❓ Вопросы:**\n"
...
# ========== АДМИН ПАНЕЛЬ ==========
async def admin_menu(upd, ctx, status_filter=None):
    if upd.effective_user.id != MANAGER_ID:
    ...
    if status_filter and status_filter != "all":
        orders = [o for o in orders if status_filter in o[2]]
    ...
    keyboard.append([InlineKeyboardButton("📊 Дашборд", callback_data="admin_dashboard")])
    keyboard.append([InlineKeyboardButton("💾 Создать бэкап", callback_data="admin_backup")])
    keyboard.append([InlineKeyboardButton("🔍 Поиск заказа", callback_data="admin_search")])
    keyboard.append([InlineKeyboardButton("🔄 Обновить", callback_data="admin_refresh")])
    
    if orders:
        unread_count = sum(1 for o in orders if o[4] == 0)
        if unread_count > 0:
...
async def admin_dashboard(upd, ctx):
    ...
    for o in orders:
        order = get_order(o[0])
        if order:
            created = datetime.fromisoformat(order['created_at']) if order['created_at'] else now
    ...
    text = (f"📊 **ДАШБОРД СТАТИСТИКИ**\n\n"
            f"📦 Всего заказов: {stats['total']}\n"
            f"💰 Общая сумма: {int(stats['total_sum'])} руб.\n\n"
...
async def admin_backup(upd, ctx):
    ...
    if backup_file:
        await q.edit_message_text(f"✅ Резервная копия создана: {os.path.basename(backup_file)}")
    else:
        await q.edit_message_text("❌ Не удалось создать резервную копию")

async def admin_search(upd, ctx):
    q = upd.callback_query
    await q.answer()
    ctx.user_data['search_mode'] = 'order'
    await q.edit_message_text("🔍 Введите номер заказа, VIN, телефон или имя клиента для поиска:")
...
async def admin_search_input(upd, ctx):
    ...
    await upd.message.reply_text(f"🔍 Результаты поиска по '{search_term}':", reply_markup=InlineKeyboardMarkup(keyboard))
    del ctx.user_data['search_mode']
...
async def admin_callback(upd, ctx):
    ...
    if data.startswith("filter_"):
        ...
        if filter_value == "all":
            ...
            orders = get_all_orders()
            unread_orders = [o for o in orders if o[4] == 0]
            await admin_menu(upd, ctx, None)
        else:
            orders = get_all_orders()
            filtered = [o for o in orders if filter_value in o[2]]
            await admin_menu(upd, ctx, filter_value)
        return
    ...
    if data.startswith("repeat_"):
        ...
        if old_order:
            ...
            await q.edit_message_text(
                ...
                f"🚗 VIN: {old_order['vin']}\n"
                f"📊 Пробег: {old_order['mileage']} км\n\n"
                f"Продолжите создание заказа:"
            )
            return MILEAGE
        await q.edit_message_text("❌ Не удалось скопировать данные")
        return
    ...
    if data.startswith("admin_order_"):
        ...
        kb = [
            ...
            [InlineKeyboardButton("💰 Установить стоимость доставки", callback_data=f"set_delivery_price_{order_num}")],
            ...
            [InlineKeyboardButton("📋 Быстрые ответы", callback_data=f"quick_replies_{order_num}")],
            [InlineKeyboardButton("◀️ Назад", callback_data="admin_back")]
        ]
        await q.edit_message_text(text, reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')
        return
    
    if data.startswith("quick_replies_"):
        order_num = data[14:]
        kb = [
            [InlineKeyboardButton("🔧 Уточнить марку/год", callback_data=f"quick_specify_car_{order_num}")],
            [InlineKeyboardButton("🔄 Оригинал или аналог?", callback_data=f"quick_original_or_analog_{order_num}")],
            [InlineKeyboardButton("✅ В наличии", callback_data=f"quick_in_stock_{order_num}")],
            [InlineKeyboardButton("📦 Под заказ", callback_data=f"quick_need_order_{order_num}")],
            [InlineKeyboardButton("◀️ Назад", callback_data=f"admin_order_{order_num}")]
        ]
        await q.edit_message_text("📋 **Быстрые ответы**\n\nВыберите шаблон:", reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')
        return
    ...
    if data.startswith("quick_"):
        ...
        reply_type = parts[1]
        order_num = parts[2]
        reply_text = QUICK_REPLIES.get(reply_type, "")
        order = get_order(order_num)
        if order and reply_text:
            await ctx.bot.send_message(order['user_id'], text=reply_text)
            await q.edit_message_text(f"✅ Быстрый ответ отправлен клиенту:\n\n{reply_text}")
        return
    ...
    if data.startswith("set_delivery_price_"):
        ...
        await q.edit_message_text(f"💰 Введите новую стоимость доставки для заказа {order_num} (в рублях):")
        return
...
async def set_delivery_price_input(upd, ctx):
    ...
    try:
        ...
        if new_price < 0:
            await upd.message.reply_text("❌ Стоимость не может быть отрицательной")
            return
        ...
        await ctx.bot.send_message(
            order['user_id'],
            text=f"💰 Стоимость доставки для заказа {order_num} установлена: {new_price} руб."
        )
        await upd.message.reply_text(f"✅ Стоимость доставки для заказа {order_num} установлена: {new_price} руб.")
        
    except ValueError:
        await upd.message.reply_text("❌ Введите целое число (стоимость в рублях)")
...
def main():
    ...
    order_conv = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^(🛒 Новый заказ)$"), new_order), CallbackQueryHandler(order_auto_callback, pattern="^order_"), CallbackQueryHandler(repeat_order, pattern="^repeat_")],
        states={
            VIN: [MessageHandler(filters.TEXT & ~filters.COMMAND, get_vin), MessageHandler(filters.PHOTO, get_vin), CallbackQueryHandler(order_auto_callback, pattern="^order_"), CallbackQueryHandler(repeat_order, pattern="^repeat_")],
    ...
    app.add_handler(CallbackQueryHandler(garage_delete_confirm, pattern="^garage_del_[^y]"))
    app.add_handler(CallbackQueryHandler(garage_delete, pattern="^garage_del_yes_"))
