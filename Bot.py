...
# Путь к базе данных - создаём папку если её нет
DB_DIR = "/data"
...
async def get_speed(upd, ctx): ctx.user_data['speed'] = upd.message.text; await upd.message.reply_text("??? Ваш город:"); return CITY
...
async def bonus_cmd(upd, ctx):
    ...
    text = f"?? **БОНУСЫ**\n?? Баланс: {int(bal)}\n?? Накоплено: {int(total)} ?\n? Начисление: {percent}%\n\n?? Градация:\n1% ? 100к | 2% ? 200к | 3% ? 300к\n4% ? 400к | 5% ? 500к | 6% ? 600к\n7% ? 700к | 8% ? 800к | 9% ? 900к\n10% ? 1М"
...
Базовая стоимость: {DELIVERY_BASE} ?
...
? **Самовывоз** — бесплатно
?? **Сторонняя ТК** — стоимость уточнит менеджер"""
    await upd.message.reply_text(text, parse_mode='Markdown')

async def help_cmd(upd, ctx):
    await upd.message.reply_text("?? **ПОМОЩЬ**\n\n/start - Главное меню\n/my_orders - Мои заказы\n/bonus - Бонусный баланс\n/referral - Реферальная ссылка\n/delivery - Тарифы доставки\n/admin - Админ-панель", parse_mode='Markdown', reply_markup=main_menu)
...
async def admin_stats(upd, ctx):
    ...
    await upd.edit_message_text(text, parse_mode='Markdown', reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("?? Назад", callback_data="adm_back")]]))
...
def main():
    ...
    print(f"?? Папка существует: {os.path.exists(DB_DIR)}")
    init_db()
